-- ============================================
-- FIX: Moderation System Migration (Safe Run)
-- ============================================

-- 1. Drop policies if they exist to avoid "already exists" errors
DROP POLICY IF EXISTS "Users can create reports" ON debate_reports;
DROP POLICY IF EXISTS "Moderators can view reports" ON debate_reports;
DROP POLICY IF EXISTS "Moderators can view restrictions" ON debate_user_restrictions;
DROP POLICY IF EXISTS "Users can view their own warnings" ON debate_warnings;

-- 2. Create Tables (IF NOT EXISTS handles duplication)
...
-- 3. Modify debate_messages (Add columns if they don't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'debate_messages' AND column_name = 'deleted_at') THEN
        ALTER TABLE debate_messages ADD COLUMN deleted_at TIMESTAMPTZ;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'debate_messages' AND column_name = 'deleted_by') THEN
        ALTER TABLE debate_messages ADD COLUMN deleted_by UUID REFERENCES auth.users(id) ON DELETE SET NULL;
    END IF;
END $$;
