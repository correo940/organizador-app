...
function TrendBadge({ current, previous, invert = false }: { current: number; previous: number; invert?: boolean }) {
    ...
    return (
        <span
            className={`inline-flex items-center gap-0.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full whitespace-nowrap ${good
...
export default function SavingsDashboardUI({
    ...
    // --- Previous month stats (for trend indicators) ---
    const prevMonthStats = React.useMemo(() => {
        ...
        return { income, expense, net, savingsRate: rate > 0 ? rate : 0 };
    ...
    const monthlyComparison = React.useMemo(() => {
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            months.push({ label: format(d, 'MMM', { locale: es }), income, expense });
        }
        return months;
    }, [recentTransactions, selectedMonth]);

    // --- Expense breakdown by concept (current month) ---
    const expenseByCategory = React.useMemo(() => {
        ...
        recentTransactions.forEach(tx => {
            ...
            const d = new Date(tx.date);
            if (d.getMonth() === m && d.getFullYear() === y) {
                const key = (tx.description || 'Otros').trim() || 'Otros';
    ...
    return (
        <motion.div
            ...
            animate="visible"
        >
            {/* HERO SECTION - Tarjeta Bancaria Premium */}
            <motion.div variants={itemVariants} className="relative overflow-hidden rounded-[2rem] bg-yellow-50/80 dark:bg-yellow-950/20 border-2 border-yellow-400 dark:border-yellow-600/60 text-slate-800 dark:text-slate-100 shadow-2xl shadow-yellow-400/10">
                <div className="relative z-10 p-6 md:p-8 flex flex-col gap-6">

                    {/* Top bar: month nav + chip */}
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <button onClick={onPrevMonth} className="w-8 h-8 flex items-center justify-center rounded-full bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-300 dark:border-yellow-700 hover:bg-yellow-200 dark:hover:bg-yellow-800/40 text-yellow-700 dark:text-yellow-400 transition-all active:scale-95">
                                <ChevronLeft className="w-4 h-4" />
                            </button>
                            <span className="text-sm font-semibold capitalize text-slate-600 dark:text-slate-300">
                                {format(selectedMonth, 'MMMM yyyy', { locale: es })}
                            </span>
                            <button onClick={onNextMonth} className="w-8 h-8 flex items-center justify-center rounded-full bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-300 dark:border-yellow-700 hover:bg-yellow-200 dark:hover:bg-yellow-800/40 text-yellow-700 dark:text-yellow-400 transition-all active:scale-95">
                                <ChevronRight className="w-4 h-4" />
                            </button>
                        </div>
                        <div className="flex items-center gap-2 bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-300 dark:border-yellow-700 rounded-xl px-3 py-1.5">
                            <Wallet className="w-3.5 h-3.5 text-yellow-600 dark:text-yellow-400" />
                            <span className="text-[10px] font-bold tracking-[0.15em] uppercase text-yellow-700 dark:text-yellow-400">Mi Economía</span>
                        </div>
                    </div>

                    {/* Balance hero */}
                    <div>
                        <p className="text-xs font-medium tracking-[0.2em] uppercase text-yellow-600/70 dark:text-yellow-500/60 mb-2">Patrimonio Total</p>
                        <motion.div
                            key={totalBalance}
                            initial={{ opacity: 0, y: 8 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.3 }}
                            className="text-5xl md:text-6xl font-black tracking-tight text-green-700 dark:text-green-400 leading-none"
                        >
                            {totalBalance.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}
                        </motion.div>
                        {onSyncAll && (
                            <button
                                onClick={() => { if (window.confirm('¿Re-sincronizar todos los saldos con el historial?')) onSyncAll(); }}
                                className="mt-2 text-[10px] text-slate-400 hover:text-yellow-600 dark:hover:text-yellow-400 inline-flex items-center gap-1 transition-colors"
                            >
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {/* Inicio */}
                        <div className="flex flex-col gap-1 p-3 rounded-2xl bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-300 dark:border-yellow-700/50">
                            <span className="text-[9px] font-bold tracking-[0.18em] uppercase text-yellow-600/70 dark:text-yellow-500/60">Inicio</span>
                            <span className="text-base font-bold text-slate-700 dark:text-slate-200 leading-none mt-0.5">
                        ...
                        <div className="flex flex-col gap-1 p-3 rounded-2xl bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-300 dark:border-yellow-700/50">
                            <div className="flex items-center gap-1 text-green-600 dark:text-green-400">
                                ...
                                <span className="text-[9px] font-bold tracking-[0.18em] uppercase">Ingresos</span>
                            </div>
                            <span className="text-base font-bold text-green-700 dark:text-green-300 leading-none mt-0.5">
                        ...
                        <div className="flex flex-col gap-1 p-3 rounded-2xl bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-300 dark:border-yellow-700/50">
                            <div className="flex items-center gap-1 text-red-500 dark:text-red-400">
                                ...
                                <span className="text-[9px] font-bold tracking-[0.18em] uppercase">Gastos</span>
                            </div>
                            <span className="text-base font-bold text-red-700 dark:text-red-300 leading-none mt-0.5">
                                -{monthlyStats.expense.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}
                            </span>
                        </div>
                        {/* Resultado */}
                        {(() => {
                            const net = monthlyStats.income - monthlyStats.expense;
                            const pos = net >= 0;
                            return (
                                <div className="flex flex-col gap-1 p-3 rounded-2xl bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-300 dark:border-yellow-700/50">
                                    <div className={`flex items-center gap-1 ${pos ? 'text-blue-600 dark:text-blue-400' : 'text-orange-500 dark:text-orange-400'}`}>
                                        <PiggyBank className="w-3 h-3" />
                                        <span className="text-[9px] font-bold tracking-[0.18em] uppercase">Resultado</span>
                                    </div>
                                    <span className={`text-base font-bold leading-none mt-0.5 ${pos ? 'text-blue-700 dark:text-blue-300' : 'text-orange-700 dark:text-orange-300'}`}>
                                        {pos ? '+' : ''}{net.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}
                                    </span>
                                </div>
                            );
                        })()}
                    ...
                    {Object.keys(accountStats).length > 0 && (
                        <div className="border-t border-yellow-300 dark:border-yellow-700/40 pt-5">
                            <p className="text-[9px] font-bold tracking-[0.2em] uppercase text-yellow-600/60 dark:text-yellow-500/50 mb-3">Desglose por cuentas</p>
                            <div className="flex gap-2.5 overflow-x-auto pb-1 scrollbar-hide snap-x">
                                {Object.entries(accountStats).map(([accountId, stats]) => {
                                    ...
                                    return (
                                        <div key={accountId} className="snap-start shrink-0 bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-300 dark:border-yellow-700/50 rounded-2xl p-3 min-w-[130px] flex flex-col gap-1.5">
                                            <div className="flex items-center gap-2">
                                                <div className="w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-bold text-white shadow-sm" style={{ backgroundColor: account.color || '#b87514' }}>
                                                    {account.bank_name?.charAt(0) || <Landmark className="w-3 h-3" />}
                                                </div>
                                                <span className="text-xs font-semibold truncate text-slate-700 dark:text-slate-300">{account.name}</span>
                                            </div>
                                            {stats.income > 0 && (
                                                <div className="flex justify-between items-center text-[11px]">
                                                    <span className="text-slate-400">Ingresos</span>
                                                    <span className="text-green-600 dark:text-green-400 font-bold">+{stats.income.toLocaleString('es-ES', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 })}</span>
                                                </div>
                                            )}
                                            {stats.expense > 0 && (
                                                <div className="flex justify-between items-center text-[11px]">
                                                    <span className="text-slate-400">Gastos</span>
                                                    <span className="text-red-600 dark:text-red-400 font-bold">-{stats.expense.toLocaleString('es-ES', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 })}</span>
                                                </div>
                                            )}
                                        </div>
                                    );
                    ...
                    {(['overview', 'accounts', 'goals', 'recurring', 'pending'] as const).map((tab) => (
                        ...
                        >
                            {activeTab === tab && (
                                <motion.div
                                    layoutId="activeTab"
                                    className={`absolute inset-0 rounded-xl ${tab === 'overview' ? 'bg-amber-500' :
                    ...
                    {activeTab === 'overview' && (
                        <div className="flex flex-col gap-6">
                            ...
                            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                                {[
                                    { label: 'Ingresos', value: monthlyStats.income, prev: prevMonthStats.income, icon: ArrowDownRight, iconBg: 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300', fmt: 'cur' as const, invert: false },
                                    { label: 'Gastos', value: monthlyStats.expense, prev: prevMonthStats.expense, icon: ArrowUpRight, iconBg: 'bg-rose-100 dark:bg-rose-900/40 text-rose-600 dark:text-rose-400', fmt: 'cur' as const, invert: true },
                                    { label: 'Ahorro Neto', value: monthlyStats.income - monthlyStats.expense, prev: prevMonthStats.net, icon: PiggyBank, iconBg: 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300', fmt: 'cur' as const, invert: false },
                                    { label: 'Tasa de Ahorro', value: monthlyStats.savingsRate || 0, prev: prevMonthStats.savingsRate, icon: Target, iconBg: 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400', fmt: 'pct' as const, invert: false },
                                ].map((kpi) => {
                                    const Icon = kpi.icon;
                                    return (
                                        <motion.div variants={itemVariants} key={kpi.label}>
                                            <Card className="border-2 border-amber-500/40 dark:border-amber-800/60 shadow-sm bg-amber-50 dark:bg-amber-950/30 backdrop-blur-md hover:shadow-md transition-all">
                                                <CardContent className="p-3.5 flex items-center justify-between">
                                                    <div className="min-w-0">
                                                        <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-0.5 block">{kpi.label}</span>
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            <span className="text-xl font-bold text-slate-800 dark:text-slate-100">
                                                                {kpi.fmt === 'pct'
                                                                    ? `${kpi.value.toFixed(1)}%`
                                                                    : kpi.value.toLocaleString('es-ES', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 })}
                                                            </span>
                                                            <TrendBadge current={kpi.value} previous={kpi.prev} invert={kpi.invert} />
                                                        </div>
                                                    </div>
                            ...
                            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                                ...
                                <motion.div variants={itemVariants} className="lg:col-span-8 flex flex-col gap-4">
                                    <Card className="border-2 border-amber-500/40 dark:border-amber-800/60 shadow-md shadow-amber-900/5 dark:shadow-none bg-amber-50 dark:bg-amber-950/30 backdrop-blur-xl">
                                                ...
                                                <div className="flex items-center gap-1 bg-slate-100/50 dark:bg-slate-800/50 p-1 rounded-md">
                                                    {['1M', '3M', '6M', 'YTD', '1A'].map((period) => (
                                                        <button
                                                            ...
                                                            className={`text-[10px] px-2 py-1 rounded font-bold transition-all ${chartTimeframe === period ? 'bg-white dark:bg-slate-700 shadow-sm text-amber-700 dark:text-amber-300' : 'text-slate-500 hover:text-slate-900 dark:hover:text-slate-300'}`}
                                                        >
                                                            {period}
                                                        </button>
                                                    ))}
                                                ...
                                                <AreaChart data={filteredChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                                    <defs>
                                                        <linearGradient id="colorValue" x1="0" y1="0" x2="0" y2="1">
                                                            <stop offset="5%" stopColor="#b87514" stopOpacity={0.6} />
                                                            <stop offset="95%" stopColor="#b87514" stopOpacity={0} />
                                                        </linearGradient>
                                                    </defs>
                                                    ...
                                                    />
                                                    <Area type="monotone" dataKey="value" stroke="#b87514" strokeWidth={3} fillOpacity={1} fill="url(#colorValue)" />
                                                </AreaChart>
                                            </ResponsiveContainer>
                                        </CardContent>
                                    </Card>
                                    ...
                                    <Card className="border-2 border-amber-500/40 dark:border-amber-800/60 shadow-md shadow-amber-900/5 dark:shadow-none bg-amber-50 dark:bg-amber-950/30 backdrop-blur-xl">
                                        <CardHeader className="py-3 px-5 border-b border-slate-100 dark:border-slate-800">
                                            <CardTitle className="text-sm font-semibold flex items-center justify-between">
                                                ...
                                                <Button variant="ghost" size="sm" className="text-[10px] h-6 px-2 hover:bg-amber-100 dark:hover:bg-amber-900/30 text-amber-700 dark:text-amber-400">Ver todos</Button>
                                            </CardTitle>
                                        </CardHeader>
                                ...
                                <motion.div variants={itemVariants} className="lg:col-span-4 flex flex-col gap-4">
                                    {/* Distribution Chart */}
                                    <Card className="border-2 border-amber-500/40 dark:border-amber-800/60 shadow-md shadow-amber-900/5 dark:shadow-none bg-amber-50 dark:bg-amber-950/30 backdrop-blur-xl flex-grow flex flex-col">
                                                    ...
                                                    <PieChart>
                                                        ...
                                                        >
                                                            {accounts.map((entry, index) => (
                                                                <Cell key={`cell-${index}`} fill={entry.color || '#b87514'} />
                                                            ))}
                                                        </Pie>
                                                        <Tooltip
                            ...
                            {/* SECONDARY CHARTS: Income vs Expense + Expense Breakdown */}
                            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                ...
                                <motion.div variants={itemVariants}>
                                    <Card className="border-2 border-amber-500/40 dark:border-amber-800/60 shadow-md shadow-amber-900/5 dark:shadow-none bg-amber-50 dark:bg-amber-950/30 backdrop-blur-xl h-full">
                                        <CardHeader className="py-4 px-5">
                                            <CardTitle className="text-sm font-semibold flex items-center gap-2 text-slate-800 dark:text-slate-200">
                                                <BarChart3 className="w-4 h-4 text-amber-600 dark:text-amber-400" />
                                                Ingresos vs Gastos
                                            </CardTitle>
                                            <CardDescription className="text-xs">Comparativa de los últimos 6 meses</CardDescription>
                                        </CardHeader>
                                        <CardContent className="h-[240px] px-2 pb-4">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <BarChart data={monthlyComparison} margin={{ top: 10, right: 10, left: -20, bottom: 0 }} barGap={2}>
                                                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" opacity={0.3} />
                                                    <XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 10, fontWeight: 600 }} dy={5} />
                                                    <YAxis axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 10 }} tickFormatter={(v) => `${v / 1000}k`} dx={5} />
                                                    <Tooltip
                                                        ...
                                                        formatter={(value: number, name) => [value.toLocaleString('es-ES', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }), name === 'income' ? 'Ingresos' : 'Gastos']}
                                                    />
                                ...
                                <motion.div variants={itemVariants}>
                                    <Card className="border-2 border-amber-500/40 dark:border-amber-800/60 shadow-md shadow-amber-900/5 dark:shadow-none bg-amber-50 dark:bg-amber-950/30 backdrop-blur-xl h-full">
                                        <CardHeader className="py-4 px-5">
                                            ...
                                            <CardDescription className="text-xs capitalize">{format(selectedMonth, 'MMMM yyyy', { locale: es })}</CardDescription>
                                        </CardHeader>
                                            ...
                                            {expenseByCategory.length === 0 ? (
                                                ...
                                                <div className="space-y-3 pt-1">
                                                    {expenseByCategory.map((cat) => {
                                                        const pct = expenseByCategory[0].value > 0 ? (cat.value / expenseByCategory[0].value) * 100 : 0;
                                                        return (
                                                            <div key={cat.name}>
                                                                <div className="flex justify-between items-center mb-1 gap-2">
                                                                    ...
                                                                    <span className="text-xs font-bold text-slate-800 dark:text-slate-100 shrink-0">
                                                                        {cat.value.toLocaleString('es-ES', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 })}
                                                                    </span>
                                                                </div>
                                                                <div className="h-2 rounded-full bg-amber-100 dark:bg-amber-950/50 overflow-hidden">
                                                                    <div className="h-full rounded-full bg-gradient-to-r from-amber-400 to-amber-600" style={{ width: `${Math.max(pct, 4)}%` }} />
                                                                </div>
                                                            </div>
                                                        );
                                                    })}
